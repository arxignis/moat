...
pub fn check_priv(addr: &str) {
    // Skip privilege check if address is empty or invalid (e.g., when using default config)
    if addr.is_empty() {
        return;
    }

    let port = match SocketAddr::from_str(addr).map(|sa| sa.port()) {
        Ok(p) => p,
        Err(_) => {
            warn!("Invalid socket address '{}', skipping privilege check", addr);
            return;
        }
    };
