...
pub struct FilterEvent {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub ip: Option<IpSection>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub threat: Option<ThreatSection>,
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub fn build_event_from_request(
    ...
    let scheme = uri
        .scheme()
        .map(|s| s.to_string())
        .or_else(|| Some(if is_https { "https" } else { "http" }.to_string()));
    // Use port from URI, or default to 443 for HTTPS, 80 for HTTP
    let port = uri
        .port_u16()
        .or_else(|| Some(if is_https { 443 } else { 80 }));
    ...
    let query_hash = query
        .as_ref()
        .map(|q| format!("{:x}", Sha256::digest(q.as_bytes())));
    ...
    let (ip_section, threat_section) = if let Some(threat) = threat_response {
        ...
        let ip_src = IpSrcSection {
            country: intel.and_then(|i| {
                i.get("geo")
                    .and_then(|g| g.get("country").and_then(|c| c.as_str().map(String::from)))
            }),
            ...
            asn_country: intel.and_then(|i| {
                i.get("geo")
                    .and_then(|g| g.get("iso_code").and_then(|c| c.as_str().map(String::from)))
            }),
        };
...
pub fn extract_captcha_response(
    ...
    if !body.is_empty() {
        if let Ok(body_str) = std::str::from_utf8(body) {
            ...
            if let Some(p) = provider.as_ref() {
                ...
                for (k, v) in pairs {
                    if k.as_ref().ends_with("captcha-response") {
                        return Some(v.into_owned());
                    }
                }
            }
...
pub fn generate_captcha_token(ip: &str, user_agent: &str, ja4: Option<&str>) -> String {
    ...
    let ip_hash = format!("{:x}", md5::compute(ip))
        .chars()
        .take(8)
        .collect::<String>();
    let ua_hash = format!("{:x}", md5::compute(user_agent))
        .chars()
        .take(8)
        .collect::<String>();
    let salt_sig = format!(
        "{:x}",
        md5::compute(format!("{}{}{}", ip, user_agent, "SECRET_SALT"))
    );
    let sig = salt_sig.chars().take(16).collect::<String>();
    let ja4_hash = ja4
        .filter(|s| !s.is_empty() && *s != "no_ssl" && *s != "unknown")
        .map(|j| {
            format!(
                "_{:8}",
                format!("{:x}", md5::compute(j))
                    .chars()
                    .take(8)
                    .collect::<String>()
            )
...
pub fn verify_captcha_token(token: &str, ip: &str, user_agent: &str, ja4: Option<&str>) -> bool {
    ...
    if chrono::Utc::now().timestamp() - ts.unwrap() >= 7200 {
        return false;
    }
    let expected_ip_hash = format!("{:x}", md5::compute(ip))
        .chars()
        .take(8)
        .collect::<String>();
    if parts[2] != expected_ip_hash {
        return false;
    }
    let expected_ua_hash = format!("{:x}", md5::compute(user_agent))
        .chars()
        .take(8)
        .collect::<String>();
    if parts[3] != expected_ua_hash {
        return false;
    }
    ...
    if parts[5] != expected_sig {
        return false;
    }
    if parts.len() >= 7 {
        if let Some(j) = ja4 {
            if !j.is_empty() && j != "no_ssl" && j != "unknown" {
                let expected_ja4_hash = format!("{:x}", md5::compute(j))
                    .chars()
                    .take(8)
                    .collect::<String>();
                if parts[6] != expected_ja4_hash {
                    return false;
                }
            }
        }
