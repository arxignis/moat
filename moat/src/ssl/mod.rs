    ...
    loop {
        ...
        match state.status {
            ...
            OrderStatus::Invalid => {
                ...
                loop {
                    let authz = match auth_results.next().await {
                        Some(Ok(authz)) => authz,
                        Some(Err(_)) => break,
                        None => break,
                    };
...
pub async fn proxy_http_service(
    ...
    if let Some(arx) = ctx.arxignis.as_ref() {
                        ...
                        if matches!(arx.mode, crate::arxignis::ArxignisMode::Monitor) {
                            ...
                            if provider.is_some() && !site_key.is_empty() && secret_ok {
                                ...
                                if let Some(res_val) = captcha_res.as_deref() {
                                    ...
                                    if valid {
                                        // CAPTCHA validated - continue to upstream
                                        // In a real implementation, we'd set a bypass cookie here
                                        // For now, we just continue to the upstream without blocking
                                    }
                                }
        ...
        if !req_body_bytes.is_empty() {
            ...
            let scan = ScanRequest { content_type, body: String::from_utf8_lossy(&req_body_bytes).to_string() };
