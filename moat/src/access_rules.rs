...
async fn fetch_access_rules(
    ...
    match response.status() {
        StatusCode::OK => {
            let body: AccessRulesApiResponse = response.json().await?;
            Ok(body)
        }
...
///   `api_key` is the Arx Ignis API key
...
fn apply_rules_to_skel(
    ...
    for rule in &resp.data {
        ...
        for ip_str in &rule.block.ips {
            if ip_str.contains(':') {
                if !ipv6_logged {
                    eprintln!("IPv6 entries ignored in IPv4 map (example: {})", ip_str);
                    ipv6_logged = true;
                }
                continue;
            }
            if let Some((net, prefix)) = parse_ipv4_ip_or_cidr(ip_str) {
                ...
                eprintln!("invalid ip/cidr ignored: {}", ip_str);
            }
        }
